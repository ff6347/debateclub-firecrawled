...
export async function scrapePendingLinks({
	...
	const firecrawl = new FirecrawlApp({
		apiKey: apiKey,
		apiUrl: firecrawlApiUrl ?? "http://localhost:3002",
	});
			...
			try {
				...
				if (scrapeResult.success) {
					...
					if (!scrapeResult.markdown) {
						throw new Error(
							"Scrape reported success but markdown content is missing.",
						);
					}
					...
					if ("keywords" in updateData) {
						if (metadata?.keywords && typeof metadata.keywords === "string") {
							updateData.keywords = metadata.keywords
								.split(",")
					...
					if (updateError) {
						console.error(
							`Error updating DB for ${link.url}:`,
							updateError.message,
						);
					}
				} else {
					const scrapeErrorMessage =
						typeof scrapeResult.error === "string"
							? scrapeResult.error
							: "Unknown scrape error (success=false)";
				...
				if (updateError) {
					console.error(
						`Error updating DB with error for ${link.url}:`,
						updateError.message,
					);
				}
			}
