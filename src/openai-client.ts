...
Return the response ONLY as a JSON object with the following structure:
{
  ...
  "tags": ["tag1", "tag2", "tag3"]
	...
	const model = openaiModel ?? "gpt-4.1-nano"; // Use the specified model

	for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
		try {
			console.log(
				`Attempting OpenAI call ${attempt}/${MAX_RETRIES} for ${sourceUrl}`,
			);
			...
			try {
				...
				if (
					...
					throw new Error(
						"Parsed JSON does not match expected structure ({summary: string, tags: string[]}).",
					);
				}
			...
			if (!summaryText && tags.length === 0) {
				throw new Error(
					"OpenAI response contained an empty summary and no tags.",
				);
			}
			...
			console.error(
				`Error calling OpenAI API (Attempt ${attempt}/${MAX_RETRIES}):`,
				error.message,
			);
			if (attempt === MAX_RETRIES) {
				// If this was the last attempt, re-throw the error
				console.error(
					`OpenAI call failed after ${MAX_RETRIES} attempts for URL: ${sourceUrl}`,
				);
