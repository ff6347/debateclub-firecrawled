...
import { callOpenAI, type OpenAIResult } from './openai-client.ts'; // Adjust path if needed
...
export async function summarizePendingContent(
	...
	console.log(`Found ${linksToSummarize.length} pieces of content to summarize.`);
	...
	const summaryPromises = linksToSummarize.map((link) =>
		limit(async () => {
			if (!link.markdown_content || !link.url || typeof link.id !== 'number') {
				console.warn(`Skipping summarization for link ID ${link.id || 'unknown'} due to missing content, URL, or ID.`);
				return;
			}
			...
			console.log(`Attempting to summarize content for link ID: ${linkId} (URL: ${link.url})`); // Log URL
			try {
				...
				const openAIResult: OpenAIResult = await callOpenAI(
					...
					pageTitle,
					pageDescription,
					existingTagNames // Pass existing tag names
				);

				// 5. Upsert tags returned by OpenAI
				const tagsToUpsert = openAIResult.tags.map((name: string) => ({ name }));
				...
				if (tagsToUpsert.length > 0) {
					const { data: upsertResult, error: upsertError } = await supabase
						.from('tags')
						.upsert(tagsToUpsert, { onConflict: 'name', ignoreDuplicates: false }) // Ensure ignoreDuplicates is false or default
						.select('id, name'); // Select to get IDs

					if (upsertError) {
						console.error(`Error upserting tags for link ID ${linkId}:`, upsertError.message);
						// Decide whether to proceed without tags or fail
						throw upsertError; // Failing for now
					}
					upsertedTags = upsertResult || [];
					 console.log(`Upserted/found ${upsertedTags.length} tags for link ID: ${linkId}`);
				}
				...
				if (deleteError) {
					console.error(`Error deleting old tag associations for link ID ${linkId}:`, deleteError.message);
					throw deleteError;
				}
				...
				if (upsertedTags.length > 0) {
					...
					if (insertLinkTagsError) {
						console.error(`Error inserting new tag associations for link ID ${linkId}:`, insertLinkTagsError.message);
				...
				if (updateLinkError) {
					console.error(`Error updating link summary for link ID ${linkId}:`, updateLinkError.message);
				...
				const errorMessage = err.message || 'Unknown summarization or tagging error';
				...
				if (updateError) {
					console.error(`Error updating DB with summarization error for link ID ${linkId}:`, updateError.message);
				}
			}
		})
	);
